...
const checkTemplate = (rightAnswers, studentAnswers, countingScore) => {
  const totalRightAnswers = (countingScore(rightAnswers, studentAnswers));
  return totalRightAnswers;
};

const countingScore = (rAnswer, uAnswer) => {
  if (rAnswer === uAnswer) {
    return 1;
  } if (uAnswer === 'N.A') {
    return 0;
